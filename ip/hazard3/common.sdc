...
foreach flop $scan_flops {
    ...
    if {[string equal $q $d]} {
        puts "(SCANMAP) Disabling hold checks -> D for pseudo-DFFE $flop_name (D = Q = ${q})"
        set_false_path -hold -to [get_pins ${flop_name}/D]
    } elseif {[string equal $q $si]} {
        puts "(SCANMAP) Disabling hold checks -> SI for pseudo-DFFE $flop_name (SI = Q = ${q})"
...
puts "Finished burning of the infidels" 
